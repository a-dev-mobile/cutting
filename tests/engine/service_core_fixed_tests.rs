//! Comprehensive tests for the fixed core service methods
//!
//! This test suite validates the corrected implementations against the original Java behavior
//! and ensures production readiness with edge case handling.

use cutlist_optimizer_cli::engine::service::core_fixed::CutListOptimizerServiceImpl;
...
mod integration_tests {
    ...
    fn test_workflow_integration() {
        ...
        let material_groups = service.get_tile_dimensions_per_material(&tiles).unwrap();
        ...
        let is_one_dim = service.is_one_dimensional_optimization(wood_tiles, &wood_stock).unwrap();
        assert!(is_one_dim); // Should be true because all share width=100

        // Test group generation
        let groups = service.generate_groups(wood_tiles, &wood_stock, &task).unwrap();
    ...
    fn test_error_propagation() {
        ...
        assert!(service.get_tile_dimensions_per_material(&empty_tiles).is_err());
        assert!(service.is_one_dimensional_optimization(&empty_tiles, &stock_tiles).is_err());
        assert!(service.generate_groups(&empty_tiles, &stock_tiles, &task).is_err());
    }
    ...
    fn test_performance_characteristics() {
        ...
        let _material_groups = service.get_tile_dimensions_per_material(&tiles).unwrap();
        let _is_one_dim = service.is_one_dimensional_optimization(&tiles, &stock_tiles).unwrap();
        let _groups = service.generate_groups(&tiles, &stock_tiles, &task).unwrap();
